...
export class AiService {
  ...
  async *generateCatMessageStream(userId: number, event?: string) {
    try {
      const count = Math.random();
      let shouldSearch = false;
      let shouldByTime = false;
      ...
      switch (event) {
        case 'drink': eventName = '你在喝水'; break;
        case 'eat': eventName = '你在吃饭'; break;
        case 'toilet': eventName = '你在上厕所'; break;
        case 'play': eventName = '你在玩耍'; break;
        case 'touch': eventName = '主人抚摸了你'; break;
        case 'click': eventName = '主人点击了你'; break;
      }

      if (
        eventName &&
        (['你在吃饭', '你在喝水', '主人抚摸了你', '你在上厕所', '你在玩耍'].includes(eventName) || count > 0.6)
      ) {
        shouldByEvent = true;
      } else if (count < 0.3) {
        shouldSearch = true;
      ...
      let searchInfo = '';
      if (shouldSearch) {
        const searchQueries = ['今天天气', '今日新闻热点', '有趣的事实', '今天是什么节日'];
